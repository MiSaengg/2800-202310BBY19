...
export async function unLikeThread({ threadId, loginUserId }) {
  try {
    ...
    mainThreadLikedByUser = mainThreadLikedByUser.filter((element) => element !== loginUserId)
...
export async function getSearchMainThreads(searchParam, genreParam, tagParam) {
  try {
    const genreFilter = genreParam !== "" ? { genre: { equals: genreParam }} : {};
...
export async function updateVotedBranchThreadInMainThread(mainThreadId, branchThreadId ,userId){
  try {
    const mainThread = await prisma.mainThread.findUnique({
      where: {
        id : mainThreadId
...
export async function updateUnVotedBranchThreadInMainThread(mainThreadId, branchThreadId, userId){
  try {
    const mainThread = await prisma.mainThread.findUnique({
      where: {
        id : mainThreadId
    ...
    userUnVotedBranchThreadArr = userUnVotedBranchThreadArr.filter((e) => e !== branchThreadId)

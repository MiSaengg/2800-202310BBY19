...
export async function mainThreadAI({ mainThreadId, title, genre, mainCharacter }) {
  try {
    const gptResponse = await axios.post(
      'https://api.openai.com/v1/engines/davinci-codex/completions',
      {
        prompt: `Given title: "${title}" , genre: "${genre}" , and number of main characters: "${mainCharacter}" , start a short story beginning.`,
        max_tokens: 60
      },
      {
        headers: {
    ...
    const thread = await prisma.mainThread.update({
      where: { id: mainThreadId },
      data: {
        pilot: gptResponse.data.choices[0].text,
      },
    });
